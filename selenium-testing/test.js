 ...
 * =====================================================
 * 135 Test Cases across 8 Modules:
 ...
 *
 * Report: Selenium_E2E_Report_TruthGuard_<timestamp>.xlsx
 * Usage : npm test
 */
...
// ─── CONFIG ───────────────────────────────────────────────────────────────────
const PORT       = 3001; // Separate port to avoid conflict with other servers
const BASE_URL   = `http://localhost:${PORT}`;
const TIMESTAMP  = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
const REPORT_FILE = path.join(__dirname, `Selenium_E2E_Report_TruthGuard_${TIMESTAMP}.xlsx`);
const WEB_APP_DIR = path.join(__dirname, '../TruthGuard-Web/web-app');
...
const C = {
  ...
  subHeaderBg: '16213E', moduleBg:    '0F3460',
  moduleText:  'E94560', passedBg:    'D4EDDA',
  passedText:  '155724', failedBg:    'F8D7DA',
  failedText:  '721C24', rowAlt:      'F8F9FA',
  rowNorm:     'FFFFFF', accent:      '0F3460',
  statPass:    '28A745', statFail:    'DC3545',
  statTotal:   '17A2B8', border:      'BDC3C7',
};
...
async function navigateTo(screen) {
  const ids = {
    home: 'nav-home', verify: 'nav-verify', trending: 'nav-trending',
    dashboard: 'nav-dashboard', about: 'nav-about',
  };
  await driver.findElement(By.id(ids[screen])).click();
...
function startServer() {
  return new Promise((resolve) => {
    console.log('\n🚀 Starting TruthGuard Web Server on port ' + PORT + '...');
...
async function runTests() {
  const opts = new chrome.Options()
    .addArguments('--no-sandbox', '--disable-gpu', '--disable-dev-shm-usage', '--window-size=1400,900');
  if (!HEADED) opts.addArguments('--headless=new');

  console.log(`\n🧪 Initializing Chrome WebDriver (${HEADED ? 'headed' : 'headless'})...`);
  ...
  await tc('Page Load', 'TC-001', 'App loads successfully',
    'Navigate to BASE_URL, URL must start with BASE_URL', async () => {
      const url = await driver.getCurrentUrl();
      if (!url.startsWith(BASE_URL)) throw new Error(`Unexpected URL: ${url}`);
    });

  await tc('Page Load', 'TC-002', 'Browser tab title correct',
    'Title must contain "TruthGuard"', async () => {
      const title = await driver.getTitle();
      if (!title.includes('TruthGuard')) throw new Error(`Got title: "${title}"`);
    });

  await tc('Page Load', 'TC-003', 'Sidebar exists in DOM',
    'aside.sidebar element present', async () => {
      await driver.findElement(By.css('aside.sidebar'));
    });

  await tc('Page Load', 'TC-004', 'Main content area exists',
    'main.main-content element present', async () => {
      await driver.findElement(By.css('main.main-content'));
    });

  await tc('Page Load', 'TC-005', 'App container wraps layout',
    'div.app-container present', async () => {
      await driver.findElement(By.css('div.app-container'));
    });

  await tc('Page Load', 'TC-006', 'All 5 screens exist in DOM',
    'All screen-* sections present', async () => {
  ...
  await tc('Page Load', 'TC-007', 'Home screen active on load',
    '#screen-home has "active" class', async () => {
  ...
  await tc('Page Load', 'TC-008', 'Only one screen active on load',
    'Exactly 1 .screen-section.active present', async () => {
  ...
  await tc('Page Load', 'TC-009', 'Viewport meta tag present',
    'meta[name="viewport"] with width=device-width', async () => {
  ...
  await tc('Page Load', 'TC-010', 'Google Fonts linked',
    'link[href*="fonts.googleapis.com"] present', async () => {
  ...
  await tc('Page Load', 'TC-011', 'CSS stylesheet linked',
    'link[href="style.css"] present', async () => {
      await driver.findElement(By.css('link[href="style.css"]'));
    });

  await tc('Page Load', 'TC-012', 'JS app module linked',
    'script[src="app.js"] present', async () => {
      await driver.findElement(By.css('script[src="app.js"]'));
    });
  ...
  await tc('Navigation', 'TC-013', 'Sidebar logo area visible',
    '.logo-area element present and displayed', async () => {
      const logo = await driver.findElement(By.css('.logo-area'));
      if (!await logo.isDisplayed()) throw new Error('Logo area not displayed');
    });

  await tc('Navigation', 'TC-014', 'Logo text is TRUTHGUARD',
    '.logo-text contains TRUTHGUARD', async () => {
      const text = await driver.findElement(By.css('.logo-text')).getText();
      if (text !== 'TRUTHGUARD') throw new Error(`Got: "${text}"`);
    });

  await tc('Navigation', 'TC-015', 'Sidebar has 5 nav links',
    '.nav-links a.nav-item count is 5', async () => {
  ...
  await tc('Navigation', 'TC-016', 'Home nav link present',
    'id=nav-home exists', async () => { await driver.findElement(By.id('nav-home')); });

  await tc('Navigation', 'TC-017', 'Verify nav link present',
    'id=nav-verify exists', async () => { await driver.findElement(By.id('nav-verify')); });

  await tc('Navigation', 'TC-018', 'Trending nav link present',
    'id=nav-trending exists', async () => { await driver.findElement(By.id('nav-trending')); });

  await tc('Navigation', 'TC-019', 'Dashboard nav link present',
    'id=nav-dashboard exists', async () => { await driver.findElement(By.id('nav-dashboard')); });

  await tc('Navigation', 'TC-020', 'About nav link present',
    'id=nav-about exists', async () => { await driver.findElement(By.id('nav-about')); });

  await tc('Navigation', 'TC-021', 'Home nav active on load',
    '#nav-home has active class initially', async () => {
  ...
  await tc('Navigation', 'TC-022', 'Click Verify nav → verify screen',
    'Verify screen becomes active', async () => {
  ...
  await tc('Navigation', 'TC-023', 'Click Trending nav → trending screen',
    'Trending screen becomes active', async () => {
  ...
  await tc('Navigation', 'TC-024', 'Click Dashboard nav → dashboard screen',
    'Dashboard screen becomes active', async () => {
  ...
  await tc('Navigation', 'TC-025', 'Click About nav → about screen',
    'About screen becomes active', async () => {
  ...
  await tc('Navigation', 'TC-026', 'Click Home nav returns to home',
    'Home screen becomes active again', async () => {
  ...
  await tc('Navigation', 'TC-027', 'Active nav class changes on click',
    'nav-verify has active class after click', async () => {
      await driver.findElement(By.id('nav-verify')).click();
  ...
  await tc('Navigation', 'TC-028', 'Previous nav loses active class',
    'nav-home loses active after clicking verify', async () => {
      const cls = await driver.findElement(By.id('nav-home')).getAttribute('class');
      if (cls.includes('active')) throw new Error('nav-home still active after navigating away');
    });

  await tc('Navigation', 'TC-029', 'Only 1 screen active after nav',
    'Exactly 1 .screen-section.active after click', async () => {
      await driver.findElement(By.id('nav-trending')).click();
      await driver.sleep(300);
      const actives = await driver.findElements(By.css('.screen-section.active'));
  ...
  await tc('Navigation', 'TC-030', 'Sidebar version label visible',
    '.sidebar-footer .version-label contains "Version"', async () => {
  ...
  await tc('Home', 'TC-031', 'Hero shield badge visible',
    '.shield-badge element present and displayed', async () => {
      const el = await driver.findElement(By.css('.shield-badge'));
      if (!await el.isDisplayed()) throw new Error('Shield badge not visible');
    });

  await tc('Home', 'TC-032', 'Hero title "TRUTHGUARD" visible',
    'h1.hero-title text is TRUTHGUARD', async () => {
      const t = await driver.findElement(By.css('h1.hero-title')).getText();
      if (t !== 'TRUTHGUARD') throw new Error(`Got: "${t}"`);
    });

  await tc('Home', 'TC-033', 'Hero subtitle contains "AI Powered"',
    '.hero-subtitle contains AI Powered', async () => {
      const t = await driver.findElement(By.css('.hero-subtitle')).getText();
      if (!t.includes('AI Powered')) throw new Error(`Got: "${t}"`);
    });

  await tc('Home', 'TC-034', 'Quick Actions card visible',
    '.quick-actions-card present and displayed', async () => {
  ...
  await tc('Home', 'TC-035', 'Quick Actions heading text correct',
    'h3 inside quick-actions-card says "Quick Actions"', async () => {
      const t = await driver.findElement(By.css('.quick-actions-card h3')).getText();
      if (!t.includes('Quick Actions')) throw new Error(`Got: "${t}"`);
    });

  await tc('Home', 'TC-036', '"Verify News" quick button exists',
    '#btn-goto-verify present', async () => { await driver.findElement(By.id('btn-goto-verify')); });

  await tc('Home', 'TC-037', '"Trending News" quick button exists',
    '#btn-goto-trending present', async () => { await driver.findElement(By.id('btn-goto-trending')); });

  await tc('Home', 'TC-038', '"View Dashboard" quick button exists',
    '#btn-goto-dashboard present', async () => { await driver.findElement(By.id('btn-goto-dashboard')); });

  await tc('Home', 'TC-039', '"About Us" quick button exists',
    '#btn-goto-about present', async () => { await driver.findElement(By.id('btn-goto-about')); });

  await tc('Home', 'TC-040', 'Mini stats row visible',
    '.mini-stats-row present and displayed', async () => {
  ...
  await tc('Home', 'TC-041', 'Home shows 2 mini stat cards',
    '.mini-stat-card count is 2', async () => {
  ...
  await tc('Home', 'TC-042', '"Verified Articles" stat element exists',
    '#stat-quick-verified present', async () => { await driver.findElement(By.id('stat-quick-verified')); });

  await tc('Home', 'TC-043', 'Initial verified count is 25',
    '#stat-quick-verified text is "25"', async () => {
      const t = await driver.findElement(By.id('stat-quick-verified')).getText();
      if (t !== '25') throw new Error(`Got: "${t}"`);
    });

  await tc('Home', 'TC-044', '"System Accuracy" stat element exists',
    '#stat-quick-accuracy present', async () => { await driver.findElement(By.id('stat-quick-accuracy')); });

  await tc('Home', 'TC-045', 'Initial system accuracy is 92%',
    '#stat-quick-accuracy text is "92%"', async () => {
      const t = await driver.findElement(By.id('stat-quick-accuracy')).getText();
      if (t !== '92%') throw new Error(`Got: "${t}"`);
    });

  await tc('Home', 'TC-046', '"Verify News" quick btn navigates to verify screen',
    'btn-goto-verify click activates screen-verify', async () => {
  ...
  await goHome();
  await tc('Home', 'TC-047', '"Trending News" quick btn navigates',
    'btn-goto-trending click activates screen-trending', async () => {
  ...
  await goHome();
  await tc('Home', 'TC-048', '"View Dashboard" quick btn navigates',
    'btn-goto-dashboard click activates screen-dashboard', async () => {
  ...
  await goHome();
  await tc('Home', 'TC-049', '"About Us" quick btn navigates',
    'btn-goto-about click activates screen-about', async () => {
  ...
  await tc('Home', 'TC-050', 'Sidebar visible on all screens',
    'aside.sidebar displayed on about screen', async () => {
  ...
  await tc('Verify', 'TC-051', 'Verify screen section exists',
    'id=screen-verify in DOM', async () => { await driver.findElement(By.id('screen-verify')); });

  await tc('Verify', 'TC-052', '"Verify News" heading visible',
    '#screen-verify h2 contains Verify', async () => {
      const t = await driver.findElement(By.css('#screen-verify h2.section-title')).getText();
      if (!t.includes('Verify')) throw new Error(`Got: "${t}"`);
    });

  await tc('Verify', 'TC-053', 'Verify screen description text present',
    '.section-desc has meaningful content', async () => {
  ...
  await tc('Verify', 'TC-054', 'Input card exists',
    '.input-card.glass-card present', async () => {
      await driver.findElement(By.css('.input-card'));
    });

  await tc('Verify', 'TC-055', 'Textarea element exists',
    'id=news-input is a <textarea>', async () => {
      const el = await driver.findElement(By.id('news-input'));
      const tag = await el.getTagName();
      if (tag !== 'textarea') throw new Error(`Expected textarea, got ${tag}`);
    });

  await tc('Verify', 'TC-056', 'Textarea has placeholder text',
    'placeholder attribute set on #news-input', async () => {
      const ph = await driver.findElement(By.id('news-input')).getAttribute('placeholder');
      if (!ph || ph.length < 5) throw new Error(`Placeholder missing: "${ph}"`);
    });

  await tc('Verify', 'TC-057', '"Paste News Here" label exists',
    'label[for="news-input"] contains Paste', async () => {
      const t = await driver.findElement(By.css('label[for="news-input"]')).getText();
      if (!t.includes('Paste')) throw new Error(`Got: "${t}"`);
    });

  await tc('Verify', 'TC-058', 'Analyze button exists',
    'id=analyze-btn button present', async () => { await driver.findElement(By.id('analyze-btn')); });

  await tc('Verify', 'TC-059', 'Analyze button text is "Analyze"',
    'analyze-btn text matches', async () => {
      const t = await driver.findElement(By.id('analyze-btn')).getText();
      if (t !== 'Analyze') throw new Error(`Got: "${t}"`);
    });

  await tc('Verify', 'TC-060', 'Result card hidden initially',
    '#result-card has "hidden" class on page load', async () => {
  ...
  await tc('Verify', 'TC-061', 'Empty submit triggers alert',
    'Alert shown when textarea is empty and Analyze clicked', async () => {
      const input = await driver.findElement(By.id('news-input'));
      await input.clear();
      await driver.findElement(By.id('analyze-btn')).click();
      await driver.sleep(300);
  ...
  // Fake keyword tests TC-062 to TC-066
  ...
  for (let i = 0; i < fakeKeywords.length; i++) {
    ...
    const id = tcIds[i];
    await tc('Verify', id, `Keyword "${kw}" triggers Fake result`,
      `Input containing "${kw}" → Likely Fake News`, async () => {
        await goHome();
  ...
  await tc('Verify', 'TC-067', 'Fake result confidence is 88%',
    'result-confidence contains 88%', async () => {
      await goHome();
  ...
  await tc('Verify', 'TC-068', 'Fake result recommendation exists',
    'result-recommendation contains "trusted"', async () => {
  ...
  await tc('Verify', 'TC-069', 'Genuine news shows "Likely Genuine News"',
    'Neutral text → Likely Genuine News result', async () => {
      await goHome();
  ...
  await tc('Verify', 'TC-070', 'Genuine result confidence is 94%',
    'result-confidence contains 94%', async () => {
      const t = await driver.findElement(By.id('result-confidence')).getText();
      if (!t.includes('94%')) throw new Error(`Got: "${t}"`);
    });

  await tc('Verify', 'TC-071', 'Genuine recommendation says "appears reliable"',
    'result-recommendation contains "reliable"', async () => {
      const t = await driver.findElement(By.id('result-recommendation')).getText();
      if (!t.toLowerCase().includes('reliable')) throw new Error(`Got: "${t}"`);
    });

  await tc('Verify', 'TC-072', 'Result card visible after analysis',
    'result-card loses "hidden" class', async () => {
      const cls = await driver.findElement(By.id('result-card')).getAttribute('class');
      if (cls.includes('hidden')) throw new Error('Result card still hidden after analysis');
    });

  await tc('Verify', 'TC-073', '"Analysis Result" heading in card',
    '#result-card h3 contains Analysis Result', async () => {
  ...
  await tc('Verify', 'TC-074', 'Result title element exists',
    'id=result-title in DOM', async () => { await driver.findElement(By.id('result-title')); });

  await tc('Verify', 'TC-075', 'Result confidence element exists',
    'id=result-confidence in DOM', async () => { await driver.findElement(By.id('result-confidence')); });

  await tc('Verify', 'TC-076', 'Result recommendation element exists',
    'id=result-recommendation in DOM', async () => { await driver.findElement(By.id('result-recommendation')); });

  await tc('Verify', 'TC-077', 'Result card has divider element',
    'hr.divider inside result-card', async () => {
      await driver.findElement(By.css('#result-card hr.divider'));
    });

  await tc('Verify', 'TC-078', 'UPPERCASE fake keyword detected',
    '"FAKE" uppercase → Likely Fake', async () => {
      await goHome();
  ...
  await tc('Verify', 'TC-079', 'Mixed-case keyword detected',
    '"FaKe" mixed case → Likely Fake', async () => {
      await goHome();
  ...
  await tc('Verify', 'TC-080', 'Keyword at end of long text detected',
    'Long text ending with "fake" still triggers Fake result', async () => {
      await goHome();
      await analyzeNews('A very long news article about many things happening in the world today and tomorrow, but it is ultimately fake');
      const t = await driver.findElement(By.id('result-title')).getText();
      if (!t.includes('Likely Fake')) throw new Error(`Got: "${t}"`);
    });

  await tc('Verify', 'TC-081', 'Re-analyze with new text works',
    'Second analysis updates result correctly', async () => {
      await goHome();
  ...
  await tc('Verify', 'TC-082', 'Fake stat increments in dashboard',
    'dashboard-fake count increases after fake analysis', async () => {
  ...
  await tc('Verify', 'TC-083', 'True stat increments in dashboard',
    'dashboard-true count increases after genuine analysis', async () => {
      await navigateTo('dashboard');
      await driver.sleep(300);
  ...
  await tc('Verify', 'TC-084', 'Total verified increments in dashboard',
    'dashboard-total increases after analysis', async () => {
      await navigateTo('dashboard');
      await driver.sleep(300);
  ...
  await tc('Verify', 'TC-085', 'Home mini-stats update after analysis',
    'stat-quick-verified increases after analysis via goHome', async () => {
      await goHome();
  ...
  await tc('Trending', 'TC-086', 'Trending screen section exists',
    'id=screen-trending in DOM', async () => { await driver.findElement(By.id('screen-trending')); });

  await tc('Trending', 'TC-087', '"Trending News" heading visible',
    'h2 contains Trending', async () => {
  ...
  await tc('Trending', 'TC-088', 'Trending description present',
    '.section-desc has content', async () => {
  ...
  await tc('Trending', 'TC-089', 'Trending list container exists',
    'id=trending-list-container present', async () => {
      await driver.findElement(By.id('trending-list-container'));
    });

  await tc('Trending', 'TC-090', 'Exactly 5 news cards rendered',
    '.news-card count is 5', async () => {
  ...
  await tc('Trending', 'TC-091', 'News cards have glass-card styling',
    '.news-card.glass-card count is 5', async () => {
  ...
  await tc('Trending', 'TC-092', 'BBC news card present',
    '.news-source contains "BBC"', async () => {
  ...
  await tc('Trending', 'TC-093', 'Reuters news card present',
    '.news-source contains "Reuters"', async () => {
      const sources = await driver.findElements(By.css('.news-source'));
      const texts = await Promise.all(sources.map(s => s.getText()));
      if (!texts.some(t => t.includes('Reuters'))) throw new Error('Reuters not found');
    });

  await tc('Trending', 'TC-094', 'NASA news card present',
    '.news-source contains "NASA"', async () => {
      const sources = await driver.findElements(By.css('.news-source'));
      const texts = await Promise.all(sources.map(s => s.getText()));
      if (!texts.some(t => t.includes('NASA'))) throw new Error('NASA not found');
    });

  await tc('Trending', 'TC-095', 'Bloomberg news card present',
    '.news-source contains "Bloomberg"', async () => {
      const sources = await driver.findElements(By.css('.news-source'));
      const texts = await Promise.all(sources.map(s => s.getText()));
      if (!texts.some(t => t.includes('Bloomberg'))) throw new Error('Bloomberg not found');
    });

  await tc('Trending', 'TC-096', 'UNESCO news card present',
    '.news-source contains "UNESCO"', async () => {
      const sources = await driver.findElements(By.css('.news-source'));
      const texts = await Promise.all(sources.map(s => s.getText()));
      if (!texts.some(t => t.includes('UNESCO'))) throw new Error('UNESCO not found');
    });

  await tc('Trending', 'TC-097', '1st card title mentions "Scientists discover"',
    'First .news-title contains "Scientists discover"', async () => {
  ...
  await tc('Trending', 'TC-098', '2nd card title mentions "AI transforming"',
    'Second .news-title contains "AI transforming"', async () => {
      const titles = await driver.findElements(By.css('.news-title'));
      const t = await titles[1].getText();
      if (!t.includes('AI transforming')) throw new Error(`Got: "${t}"`);
    });

  await tc('Trending', 'TC-099', '3rd card title mentions "Space mission"',
    'Third .news-title contains "Space mission"', async () => {
      const titles = await driver.findElements(By.css('.news-title'));
      const t = await titles[2].getText();
      if (!t.includes('Space mission')) throw new Error(`Got: "${t}"`);
    });

  await tc('Trending', 'TC-100', '4th card title mentions "Global economy"',
    'Fourth .news-title contains "Global economy"', async () => {
      const titles = await driver.findElements(By.css('.news-title'));
  ...
  await tc('Dashboard', 'TC-101', 'Dashboard screen section exists',
    'id=screen-dashboard in DOM', async () => { await driver.findElement(By.id('screen-dashboard')); });

  await tc('Dashboard', 'TC-102', '"Dashboard" heading visible',
    'h2 contains Dashboard', async () => {
  ...
  await tc('Dashboard', 'TC-103', 'Dashboard description visible',
    '.section-desc present with content', async () => {
      const t = await driver.findElement(By.css('#screen-dashboard .section-desc')).getText();
      if (!t || t.length < 5) throw new Error('Description missing');
    });

  await tc('Dashboard', 'TC-104', 'Stats grid has 4 cards',
    '.stats-grid contains 4 .stat-card elements', async () => {
  ...
  await tc('Dashboard', 'TC-105', '"Articles Verified" label visible',
    '.stat-label contains "Articles Verified"', async () => {
  ...
  await tc('Dashboard', 'TC-106', '"True News" label visible',
    '.stat-label contains "True News"', async () => {
      const labels = await driver.findElements(By.css('.stat-label'));
      const texts = await Promise.all(labels.map(l => l.getText()));
      if (!texts.some(t => t.includes('True News'))) throw new Error('True News label not found');
    });

  await tc('Dashboard', 'TC-107', '"Fake News" label visible',
    '.stat-label contains "Fake News"', async () => {
      const labels = await driver.findElements(By.css('.stat-label'));
      const texts = await Promise.all(labels.map(l => l.getText()));
      if (!texts.some(t => t.includes('Fake News'))) throw new Error('Fake News label not found');
    });

  await tc('Dashboard', 'TC-108', 'Accuracy stat label visible',
    '.stat-label contains "Accuracy"', async () => {
      const labels = await driver.findElements(By.css('.stat-label'));
      const texts = await Promise.all(labels.map(l => l.getText()));
      if (!texts.some(t => t.toLowerCase().includes('accuracy'))) throw new Error('Accuracy label not found');
    });

  await tc('Dashboard', 'TC-109', '#dashboard-total element exists',
    'id=dashboard-total in DOM', async () => { await driver.findElement(By.id('dashboard-total')); });

  await tc('Dashboard', 'TC-110', '#dashboard-true element exists',
    'id=dashboard-true in DOM', async () => { await driver.findElement(By.id('dashboard-true')); });

  await tc('Dashboard', 'TC-111', '#dashboard-fake element exists',
    'id=dashboard-fake in DOM', async () => { await driver.findElement(By.id('dashboard-fake')); });

  await tc('Dashboard', 'TC-112', '#dashboard-accuracy element exists',
    'id=dashboard-accuracy in DOM', async () => { await driver.findElement(By.id('dashboard-accuracy')); });

  await tc('Dashboard', 'TC-113', 'Accuracy value ends in %',
    'dashboard-accuracy text contains "%"', async () => {
      const t = await driver.findElement(By.id('dashboard-accuracy')).getText();
      if (!t.includes('%')) throw new Error(`Got: "${t}"`);
    });

  await tc('Dashboard', 'TC-114', 'Dashboard details card visible',
    '.dashboard-details present and displayed', async () => {
  ...
  await tc('Dashboard', 'TC-115', '"Cloud Firestore" text in details',
    '.dashboard-details contains "Cloud Firestore"', async () => {
  ...
  await tc('About', 'TC-116', 'About screen section exists',
    'id=screen-about in DOM', async () => { await driver.findElement(By.id('screen-about')); });

  await tc('About', 'TC-117', 'About icon visible',
    '.about-icon present and displayed', async () => {
      const el = await driver.findElement(By.css('.about-icon'));
      if (!await el.isDisplayed()) throw new Error('About icon not displayed');
    });

  await tc('About', 'TC-118', '"TruthGuard" about title visible',
    'h2.about-title text is TruthGuard', async () => {
      const t = await driver.findElement(By.css('h2.about-title')).getText();
      if (t !== 'TruthGuard') throw new Error(`Got: "${t}"`);
    });

  await tc('About', 'TC-119', 'System Information heading visible',
    '.about-details h3 contains "System Information"', async () => {
      const t = await driver.findElement(By.css('.about-details h3')).getText();
      if (!t.includes('System Information')) throw new Error(`Got: "${t}"`);
    });

  await tc('About', 'TC-120', 'About info list has 4 items',
    '.about-info-list li count is 4', async () => {
  ...
  await tc('About', 'TC-121', '"AI Powered Fake News Detection App" text visible',
    '.about-details contains the description text', async () => {
  ...
  await tc('About', 'TC-122', 'Version "1.0" visible',
    '.about-details contains "1.0"', async () => {
      const t = await driver.findElement(By.css('.about-details')).getText();
      if (!t.includes('1.0')) throw new Error(`Version not found`);
    });

  await tc('About', 'TC-123', '"Educational Purpose" text visible',
    '.about-details contains "Educational Purpose"', async () => {
      const t = await driver.findElement(By.css('.about-details')).getText();
      if (!t.includes('Educational Purpose')) throw new Error(`Got: "${t}"`);
    });

  await tc('About', 'TC-124', '"Node.js" in technology stack',
    '.about-details contains "Node.js"', async () => {
      const t = await driver.findElement(By.css('.about-details')).getText();
      if (!t.includes('Node.js')) throw new Error(`Node.js not in text`);
    });

  await tc('About', 'TC-125', '"Firebase" in technology stack',
    '.about-details contains "Firebase"', async () => {
      const t = await driver.findElement(By.css('.about-details')).getText();
      if (!t.includes('Firebase')) throw new Error(`Firebase not in text`);
    });

  await tc('About', 'TC-126', 'Copyright text visible',
    '.about-copyright contains "TruthGuard 2026"', async () => {
      const t = await driver.findElement(By.css('.about-copyright')).getText();
      if (!t.includes('TruthGuard 2026')) throw new Error(`Got: "${t}"`);
    });

  await tc('About', 'TC-127', '"©" copyright symbol present',
    '.about-copyright contains ©', async () => {
      const t = await driver.findElement(By.css('.about-copyright')).getText();
      if (!t.includes('©')) throw new Error(`Copyright symbol missing`);
    });

  await tc('About', 'TC-128', 'About details glass card present',
    '.about-details element exists', async () => { await driver.findElement(By.css('.about-details')); });

  await tc('About', 'TC-129', '"Description:" label in about list',
    '.about-details contains "Description:"', async () => {
      const t = await driver.findElement(By.css('.about-details')).getText();
      if (!t.includes('Description:')) throw new Error(`Description label missing`);
    });

  await tc('About', 'TC-130', '"Version:" label in about list',
    '.about-details contains "Version:"', async () => {
      const t = await driver.findElement(By.css('.about-details')).getText();
  ...
  await tc('E2E Flow', 'TC-131', 'Full fake news flow: Home→Verify→Dashboard',
    'Fake count in dashboard increments after full flow', async () => {
      await goHome();
      await navigateTo('dashboard');
  ...
  await tc('E2E Flow', 'TC-132', 'Full genuine flow: Home→Verify→Dashboard',
    'True count increments after genuine analysis', async () => {
      await goHome();
      await navigateTo('dashboard');
  ...
  await tc('E2E Flow', 'TC-133', 'Stats consistency: true+fake equals total',
    'dashboard-true + dashboard-fake === dashboard-total', async () => {
      await goHome();
      await navigateTo('dashboard');
  ...
  await tc('E2E Flow', 'TC-134', 'Navigate all 5 screens sequentially',
    'All 5 screens activate without error in order', async () => {
      await goHome();
  ...
  await tc('E2E Flow', 'TC-135', 'Home mini-stats and dashboard total match',
    'stat-quick-verified === dashboard-total after navigation', async () => {
      await goHome();
...
async function generateReport() {
  ...
  titleCell.font      = { name: 'Outfit', size: 18, bold: true, color: { argb: `FF${C.headerText}` } };
  ...
  subCell.value     = `Generated: ${new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })}  |  Engine: Node.js + Selenium WebDriver 4.x  |  Browser: Google Chrome`;
  subCell.font      = { name: 'Plus Jakarta Sans', size: 10, color: { argb: 'FFB0C4DE' } };
  subCell.fill      = { type: 'pattern', pattern: 'solid', fgColor: { argb: `FF${C.subHeaderBg}` } };
  ...
  // KPI boxes
  const kpis = [
    { label: 'TOTAL TESTS', value: total,        fg: C.statTotal, col: 'B' },
    { label: 'PASSED',      value: passed,        fg: C.statPass,  col: 'D' },
    { label: 'FAILED',      value: failed,        fg: C.statFail,  col: 'F' },
    { label: 'PASS RATE',   value: `${passRate}%`, fg: C.accent,   col: 'H' },
  ];
  ...
  // Execution details
  const detailsData = [
    ['Test Suite',       'TruthGuard Web Application — Selenium E2E'],
    ['Target URL',       BASE_URL],
  ...
  const detTitle = summary.addRow(['', 'EXECUTION DETAILS', '', '', '', '', '', '']);
  summary.mergeCells(`B${detTitle.number}:H${detTitle.number}`);
  summary.getCell(`B${detTitle.number}`).font = { name: 'Plus Jakarta Sans', bold: true, size: 12, color: { argb: `FF${C.accent}` } };
  summary.getCell(`B${detTitle.number}`).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFE8EEF7' } };
  detTitle.height = 26;
  ...
  // Module breakdown
  summary.addRow([]);
  const modTitle = summary.addRow(['', 'MODULE BREAKDOWN', '', '', '', '', '', '']);
  summary.mergeCells(`B${modTitle.number}:H${modTitle.number}`);
  summary.getCell(`B${modTitle.number}`).font = { name: 'Plus Jakarta Sans', bold: true, size: 12, color: { argb: `FF${C.accent}` } };
  summary.getCell(`B${modTitle.number}`).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFE8EEF7' } };
  modTitle.height = 26;

  const modHeader = summary.addRow(['', 'Module', '', 'Total', 'Passed', 'Failed', 'Pass Rate', '']);
  modHeader.height = 22;
  ['B','D','E','F','G'].forEach(col => {
    const c = summary.getCell(`${col}${modHeader.number}`);
    c.font      = { name: 'Plus Jakarta Sans', bold: true, size: 10, color: { argb: 'FFFFFFFF' } };
    c.fill      = { type: 'pattern', pattern: 'solid', fgColor: { argb: `FF${C.accent}` } };
  ...
  for (const mod of modules) {
    ...
    const mPass = mr.filter(r => r.status === 'PASSED').length;
    const mFail = mr.filter(r => r.status === 'FAILED').length;
    const mRate = Math.round((mPass / mr.length) * 100);
    const mRow  = summary.addRow(['', mod, '', mr.length, mPass, mFail, `${mRate}%`, '']);
    mRow.height = 20;
    mRow.getCell(2).font = { name: 'Plus Jakarta Sans', size: 10, bold: true };
    [4,5,6,7].forEach(col => mRow.getCell(col).alignment = { horizontal: 'center' });
    mRow.getCell(5).font = { name: 'Plus Jakarta Sans', size: 10, color: { argb: `FF${C.statPass}` } };
    mRow.getCell(6).font = { name: 'Plus Jakarta Sans', size: 10, color: { argb: `FF${C.statFail}` } };
  }

  summary.columns = [
    { width: 2 }, { width: 32 }, { width: 4 }, { width: 18 },
  ...
  tcTitle.value     = '🛡️  TRUTHGUARD — ALL 135 TEST CASES';
  tcTitle.font      = { name: 'Outfit', size: 15, bold: true, color: { argb: `FF${C.headerText}` } };
  tcTitle.fill      = { type: 'pattern', pattern: 'solid', fgColor: { argb: `FF${C.headerBg}` } };
  ...
  tcInfo.fill      = { type: 'pattern', pattern: 'solid', fgColor: { argb: `FF${C.subHeaderBg}` } };
  ...
  hRow.eachCell(cell => {
    ...
    cell.border    = { bottom: { style: 'medium', color: { argb: `FF${C.moduleText}` } } };
  });
  ...
  for (const r of results) {
    ...
    if (r.module !== currentModule) {
      ...
      sepRow.eachCell(cell => {
        cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: `FF${C.moduleBg}` } };
        cell.font = { name: 'Plus Jakarta Sans', bold: true, size: 10, color: { argb: `FF${C.moduleText}` } };
      });
    }
    const isPass  = r.status === 'PASSED';
    const fillCol = isPass ? C.passedBg : C.failedBg;
    const statFg  = isPass ? C.passedText : C.failedText;
    const rowBg   = rowIndex % 2 === 0 ? C.rowAlt : C.rowNorm;
    ...
    dataRow.eachCell((cell, colNum) => {
      ...
      if (colNum === 8 && r.error) {
        cell.font      = { name: 'Plus Jakarta Sans', size: 9, color: { argb: `FF${C.failedText}` } };
  ...
  tcSheet.columns = [
    ...
    { width: 52 }, { width: 14 }, { width: 14 }, { width: 55 },
  ];

  // ── SHEET 3: Failed Tests (only if any) ─────────────────────────────────────
  ...
  if (failedResults.length > 0) {
    ...
    fTitle.value     = `❌  FAILED TEST CASES — ${failedResults.length} Failure(s)`;
    fTitle.font      = { name: 'Outfit', size: 14, bold: true, color: { argb: 'FFFFFFFF' } };
    fTitle.fill      = { type: 'pattern', pattern: 'solid', fgColor: { argb: `FF${C.statFail}` } };
    ...
    failedResults.forEach((r, i) => {
      ...
      row.eachCell((cell, col) => {
        ...
        cell.fill   = { type: 'pattern', pattern: 'solid', fgColor: { argb: `FF${C.failedBg}` } };
        if (col === 5) { cell.font = { name: 'Plus Jakarta Sans', bold: true, size: 10, color: { argb: `FF${C.failedText}` } }; cell.alignment = { horizontal: 'center' }; }
        if (col === 7) { cell.alignment = { wrapText: true }; cell.font = { name: 'Plus Jakarta Sans', size: 9, color: { argb: `FF${C.failedText}` } }; }
      });
    });
    failSheet.columns = [{ width: 5 },{ width: 10 },{ width: 16 },{ width: 42 },{ width: 12 },{ width: 14 },{ width: 62 }];
  }
...
function generateMarkdownSummary() {
  ...
  md += `> **Generated:** ${timestamp} &nbsp;|&nbsp; **Browser:** Google Chrome &nbsp;|&nbsp; **Engine:** Node.js + Selenium WebDriver 4.x\n\n`;
  md += `---\n\n`;
  md += `## 📊 Results Summary\n\n`;
  ...
  for (const mod of modules) {
    ...
    const mRate = Math.round((mPass / mr.length) * 100);
    const icon  = mFail === 0 ? '✅' : '❌';
    md += `| ${icon} ${mod} | ${mr.length} | ${mPass} | ${mFail} | ${mRate}% |\n`;
  }
  md += `\n`;
  ...
  md += `---\n`;
  md += `*Excel report saved as \`${path.basename(REPORT_FILE)}\` in this directory.*\n`;
...
async function main() {
  ...
  console.log('   135 Test Cases  |  8 Modules  |  Excel Report');
  console.log('═══════════════════════════════════════════════════════════');
  ...
  try {
    ...
    console.log(`  ⏱️  Duration: ${duration}s  |  Pass Rate: ${total > 0 ? Math.round((passed/total)*100) : 0}%`);
    console.log('═══════════════════════════════════════════════════════════');

    await generateReport();
    generateMarkdownSummary();

    console.log('\n✨ Done! Open the .xlsx report for full details.\n');
